...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(99, 93, 250, 200).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(window->Size.x - 40, label_size.y + 15));
    ...
    if (Enabled)
    {
        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), 5);

        PushFont(FontAwesomeSolid);
        window->DrawList->AddText(total_bb.Min + ImVec2(15.f - CalcTextSize(Icon).x / 2.f, total_bb.GetHeight() / 2.f - CalcTextSize(Icon).y / 2.f), ImColor(1.f, 1.f, 1.f, GetStyle().Alpha), Icon);
        PopFont();

        window->DrawList->AddText(total_bb.Min + ImVec2(30.f, total_bb.GetHeight() / 2.f - CalcTextSize(Label).y / 2.f), ImColor(1.f, 1.f, 1.f, GetStyle().Alpha), Label);
    }
    else
    {
        PushFont(FontAwesomeSolid);
        window->DrawList->AddText(total_bb.Min + ImVec2(15.f - CalcTextSize(Icon).x / 2.f, total_bb.GetHeight() / 2.f - CalcTextSize(Icon).y / 2.f), ImColor(1.f, 1.f, 1.f, 0.4f * GetStyle().Alpha), Icon);
        PopFont();

        window->DrawList->AddText(total_bb.Min + ImVec2(30.f, total_bb.GetHeight() / 2.f - CalcTextSize(Label).y / 2.f), ImColor(1.f, 1.f, 1.f, 0.4f * GetStyle().Alpha), Label);
    }
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImVec2(label_size.x + 20, 40);
    ...
    static std::unordered_map<int, TabHeaderAnim> Values;
    auto Value = Values.find(TabIndex);

    if (Value == Values.end())
    {
        Values.insert({ TabIndex, TabHeaderAnim() });
        Value = Values.find(TabIndex);
    }

    if (enabled)
    {
        Value->second.selectedpox = pos.x;
    }

    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held, ImGuiButtonFlags_None);

    ImColor BGColor = enabled ? ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)) : ImColor(16, 16, 16, (int)(GetStyle().Alpha * 255));
    ImColor TextColor = enabled ? ImColor(255, 255, 255, (int)(GetStyle().Alpha * 255)) : ImColor(1.f, 1.f, 1.f, 0.4f * GetStyle().Alpha);

    static ImDrawFlags Corner;

    Value->second.posx = ImLerp(Value->second.posx, Value->second.selectedpox - window->Pos.x, 0.05f);


    if (direction == 0)
    {
        Corner = ImDrawFlags_RoundCornersAll;
    }
    else if (direction == 1)
    {
    ...
    {
        Corner = ImDrawFlags_RoundCornersNone;
    }
    else if (direction == 3)
    {
        Corner = ImDrawFlags_RoundCornersRight;
    }

    if (enabled)
        window->DrawList->AddRectFilled(ImVec2(window->Pos.x + Value->second.posx + 1, bb.Min.y + 1), ImVec2(window->Pos.x + Value->second.posx + size.x - 1, bb.Max.y - 1), ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), 5, Corner);

    window->DrawList->AddRect(bb.Min, bb.Max, ImGui::GetColorU32(ImGuiCol_Border), 5, Corner);

    window->DrawList->AddText(ImVec2(bb.Min.x + 10, bb.GetCenter().y - label_size.y / 2), TextColor, ButtonName);
    PopFont();
...
bool ImGui::TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImVec2(window->Size.x - 40, 55);
    const ImRect bb(pos, pos + size);
    ...
    {
        WidhtMax = WidhtMax + 20 + CalcTextSize(Tabs.at(i)).x;
    }
    PopFont();


    SetCursorPos(ImVec2(-100, 15) + ImVec2(bb.GetWidth() / 2 - WidhtMax / 2, 12.5f));

    window->DrawList->AddRectFilled(GetWindowPos() + GetCursorPos() + ImVec2(1, 1), GetWindowPos() + GetCursorPos() + ImVec2(WidhtMax, 40) - ImVec2(1, 1), ImColor(16, 16, 16, (int)(GetStyle().Alpha * 255)), 5, ImDrawFlags_RoundCornersAll);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_Y)
        window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, ImColor(99, 93, 250, 0), ImColor(99, 93, 250, 0), ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (std::abs(Value->second.a) < 0.9f)
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(check_bb.GetWidth() / 2 * Value->second.a + 1, check_bb.GetHeight() / 2 * Value->second.a + 1), check_bb.Max - ImVec2(check_bb.GetWidth() / 2 * Value->second.a + 1, check_bb.GetHeight() / 2 * Value->second.a + 1), ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2), Value->second.a, ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), 360);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 2),
        window->DC.CursorPos + ImVec2(w, label_size.y + 2 + slider_height));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 0), ImVec2(frame_bb.Min.x + Value->second.anim + 1, frame_bb.Max.y), ImColor(99, 93, 250, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.border = ImLerp(it_anim->second.border, IsItemActive() || hovered ? ImColor(99, 93, 250, 255) : ImColor(30, 30, 30), g.IO.DeltaTime * 3.f);
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        //GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 99, 93, 250));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    if (out_text_clipped)
    {
        *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
        //draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255, 99, 93, 250) : IM_COL32(0, 255, 0, 255));
    }
