...
connectionopened: //used for relative URI redirect where only path (not host) changes
	...
	if (doctype == "<!doctype html") {
		...
		output.text = output.raw; //if no html found, .text = .raw		
	}
...
static inline std::map<std::string, std::string> handleURI(wrap::req& request, wrap::Response& output, INTERNET_PORT& port) {
	...
	//path (includes fragments, params etc)
	size_t path_pos = URI["host"].find_first_of("/?");
	if (path_pos != std::string::npos) {
		URI["path"] = URI["host"].substr(path_pos);
		URI["host"] = URI["host"].substr(0, path_pos);

		//extract filename if any
		size_t last_slash = URI["path"].find_last_of("/");
		if (last_slash != std::string::npos && last_slash + 1 < URI["path"].length()) {
			std::string file_part = URI["path"].substr(last_slash + 1);
			if (file_part.find(".") != std::string::npos) {
				URI["urlfile"] = file_part;
			}
		}
	}
	...
	if (URI["host"].find(":") != std::string::npos) {
		std::string port_str = URI["host"].substr(URI["host"].find(":") + 1);
		URI["host"] = URI["host"].substr(0, URI["host"].find(":"));
		if (!port_str.empty()) {
			port = (INTERNET_PORT)std::stoi(port_str);
		}
	}
